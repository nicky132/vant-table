        ...
        <template #expanded="{ row, rowIndex }">
          <div style="padding: 20px; background: #f8f9fa;">
            ...
            <div style="display: grid; grid-template-columns: repeat(2, 1fr); gap: 12px; margin-top: 12px;">
              ...
              <div><strong>状态:</strong> {{ row.status }}</div>
              <div><strong>注册时间:</strong> {{ new Date().toLocaleDateString() }}</div>
            </div>
          </div>
        </template>
      </VantTable>
...
const columns = ref([
  ...
  { key: 'phone', label: '电话',sortable: true, filterable: true},
...
const handleExpandChange = (eventData: any) => {
  console.log('展开状态变化:', eventData)
  showToast(`${eventData.expanded ? '展开' : '收起'}了第 ${eventData.rowIndex + 1} 行`)
}

// 新增：滚动边界事件处理函数
const handleScrollToTop = (eventData: any) => {
  console.log('纵向滚动到顶部:', eventData)
  addOperationHistory('滚动到顶部', '纵向滚动到表格顶部', 'info')
}

const handleScrollToBottom = (eventData: any) => {
  console.log('纵向滚动到底部:', eventData)
  addOperationHistory('滚动到底部', '纵向滚动到表格底部', 'info')
}

const handleScrollToLeft = (eventData: any) => {
  console.log('横向滚动到左边界:', eventData)
  addOperationHistory('滚动到左边界', '横向滚动到表格左侧边界', 'info')
}

const handleScrollToRight = (eventData: any) => {
  console.log('横向滚动到右边界:', eventData)
  addOperationHistory('滚动到右边界', '横向滚动到表格右侧边界', 'info')
}

const handleScroll = (eventData: any) => {
  console.log('表格滚动:', {
    scrollTop: eventData.scrollTop,
    scrollLeft: eventData.scrollLeft,
    isAtTop: eventData.scrollTop <= 5,
    isAtBottom: eventData.scrollHeight - eventData.scrollTop - eventData.clientHeight <= 5,
    isAtLeft: eventData.scrollLeft <= 5,
    isAtRight: eventData.scrollWidth - eventData.scrollLeft - eventData.clientWidth <= 5
  })
}
